...
void main()
{
	...
	diff = max(dot(normal, lightdir), 0.0); // Lambertian diffuse reflection

	// Calculate the specular lighting using Phong reflection model
	vec3 viewDir = normalize(-fragPos); // View direction
	vec3 reflectDir = reflect(-lightdir, normal); // Perfect reflection direction
	spec = pow(max(dot(viewDir, reflectDir), 0.0), phongExp); // Specular reflection
